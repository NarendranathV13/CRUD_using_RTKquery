...
const Details = () => {
    const {
        ...
        isSuccess,
        isError,
        error } = useGetDataQuery() //to get the data from api

    const [deleteData] = useDeleteDataMutation();// assigning delete method
    const [editData] = useEditDataMutation();// assigning edit method
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await deleteData({ id });// assign id to delete
            } catch (error) {
